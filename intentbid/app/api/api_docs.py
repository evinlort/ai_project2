...
API_DOCUMENTATION: List[Dict[str, object]] = [
    ...
    },
    {
        ...
        "description": {"en": "Registers a webhook that receives signed offer.created events with retries managed by the outbox.", "ru": "Регистрирует webhook, который получает подписанные события offer.created и обрабатывается через outbox."},
        "auth": {"en": "X-API-Key required", "ru": "Требуется X-API-Key"},
    ...
    },
    {
        ...
        "description": {"en": "Returns every offer for the RFO together with score and explain payload.", "ru": "Отдает все офферы по RFO с оценкой и explain payload."},
        "auth": {"en": "X-Buyer-API-Key required", "ru": "Требуется X-Buyer-API-Key"},
    ...
    },
    {
        ...
        "description": {"en": "Creates a request for offer with constraints and weighted preferences.", "ru": "Создает RFO с ограничениями и весами предпочтений."},
        "auth": {"en": "None", "ru": "Не требуется"},
    ...
    },
    {
        ...
        "description": {"en": "Returns the RFO plus the current offer count.", "ru": "Возвращает RFO и количество офферов."},
        "auth": {"en": "None", "ru": "Не требуется"},
    ...
    },
    {
        ...
        "description": {"en": "Returns the top offers with score breakdowns via the explain field.", "ru": "Отдает лучшие офферы с разбивкой скоринга в explain."},
        "auth": {"en": "None", "ru": "Не требуется"},
    ...
    },
    {
        ...
        "description": {"en": "Uploads an offer for an open RFO and runs validation before scoring.", "ru": "Отправляет оффер для открытого RFO и проверяет данные перед скорингом."},
        "auth": {"en": "X-API-Key required", "ru": "Требуется X-API-Key"},
    ...
    },
    {
        ...
        "description": {"en": "Transitions the RFO from OPEN to CLOSED and prevents new offers.", "ru": "Переводит RFO из OPEN в CLOSED и блокирует новые офферы."},
        "auth": {"en": "X-API-Key required", "ru": "Требуется X-API-Key"},
    ...
    },
    {
        ...
        "description": {"en": "Marks an offer as the winner and moves the RFO to AWARDED.", "ru": "Выбирает оффер и переводит RFO в AWARDED."},
        "auth": {"en": "X-API-Key required", "ru": "Требуется X-API-Key"},
    ...
    },
    {
        ...
        "description": {"en": "Reopens a CLOSED/AWARDED RFO where business rules allow it.", "ru": "Повторно открывает CLOSED/AWARDED RFO, если правила позволяют."},
        "auth": {"en": "X-API-Key required", "ru": "Требуется X-API-Key"},
    ...
    },
    {
        ...
        "description": {"en": "Updates the scoring_version and per-component weights for an RFO.", "ru": "Обновляет scoring_version и веса компонентов для RFO."},
        "auth": {"en": "X-API-Key required", "ru": "Требуется X-API-Key"},
    ...
    },
    {
        ...
        "description": {"en": "Shows the resolved scoring version, component breakdown, and active penalties for an RFO.", "ru": "Показывает версию скоринга, разбивку по компонентам и активные штрафы."},
        "auth": {"en": "X-API-Key required", "ru": "Требуется X-API-Key"},
