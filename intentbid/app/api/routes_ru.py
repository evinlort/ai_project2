...
templates = Jinja2Templates(directory=str(TEMPLATES_DIR / "ru"))
...
def dashboard_login_submit(
    ...
    response = RedirectResponse(url="/ru/dashboard/rfos", status_code=303)
...
def dashboard_submit_offer(
    ...
    if not rfo:
        raise HTTPException(status_code=404, detail="RFO not found")
    if rfo.status != "OPEN":
        return RedirectResponse(url=f"/ru/dashboard/rfos/{rfo_id}", status_code=303)
